...
CHROMAPRINT_CMAKE=$(find "${ANDROID_SDK_ROOT}"/cmake -path \*/bin/cmake -type f | sort -V | head -n 1)
if [[ -z ${CHROMAPRINT_CMAKE} ]]; then
  CHROMAPRINT_CMAKE=$(command -v cmake)
fi

"${CHROMAPRINT_CMAKE}" -Wno-dev \
  -G "Unix Makefiles" \
